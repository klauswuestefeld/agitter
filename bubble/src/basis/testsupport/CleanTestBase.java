...
public abstract class CleanTestBase extends AssertUtils {
	...
	private final ThreadLocal<Level> logLevelByThread = new ThreadLocal<Level>();
	...
	protected File tmpFolder() {
		if(_tmpFolder==null) { _tmpFolder = createFolder(tmpFolderName()); }
	...
	private File createFolder(String folderName) {
		...
		if(!result.exists()) { assertTrue("Unable to create tmp folder: "+result, result.mkdirs()); }
		return result;
	}
	...
	protected void assertTmpFilesExist(String... fileNames) {
		for(String fileName : fileNames) { assertTmpFileExists(fileName); }
	}


	protected void assertTmpFilesDontExist(String... fileNames) {
		for(String fileName : fileNames) { assertTmpFileDoesntExist(fileName); }
	}
	...
	public void afterCleanTest() {
		...
		getDefaultLogger().setLevel(logLevelByThread.get());
	...
	private Logger getDefaultLogger() {return Logger.getLogger("");}
	...
	private void checkThreadLeak() {
		...
		for(Thread thread : activeThreadsAfterTest) { stopIfNecessary(thread); }
	}
	...
	private void stopIfNecessary(Thread thread) {
		...
		if(thread.getName().indexOf("AWT")!=-1) {
			return; //Fix: Check for leaking Gui resources too.
		}
		if(thread.getName().indexOf("Java2D")!=-1) {
			return; //Fix: Check for leaking Gui resources too.
		}
		if(thread.getName().indexOf("SunPKCS11-Darwin")!=-1) {
			return; //Fix: Check for leaking SunPKCS11-Darwin resources too (bug on mac OSX).
		}
	...
	void deleteFolder(File folder) throws IOException {
		...
		if(!folder.isDirectory()) { throw new IllegalArgumentException(folder.getAbsolutePath()+" is not a folder"); }

		recursiveDelete(folder);

		if(!folder.delete()) { throw new IOException("Unable to delete folder: "+folder.getAbsolutePath()); }
	}
	...
	void failedWith(Method method, Throwable thrown) {
		...
		{ tryToWaitForTheFailureFromTheActualTestThread(method, thrown); } else { keepFailure(method, thrown); }
	}
	...
	protected void createTmpFiles(String... fileNames) throws IOException {
		for(String fileName : fileNames) { createTmpFile(fileName); }
	}
	...
	protected void createTmpFilesWithFileNameAsContent(String... fileNames) throws IOException {
		for(String fileName : fileNames) { createTmpFileWithFileNameAsContent(fileName); }
	}
