...
public class LuceneBase {
	...
	public void create(Events events, User owner) throws IOException {
		this.events = events;
		this.owner = owner;
		...
		Set<Event> alreadyIndexed = new HashSet<Event>();
		for(EventOcurrence oc : events.toHappen(owner)) {
			Event event = oc.event();
			if(alreadyIndexed.add(event))
				addEvent(event);
	...
	public List<Event> search(String querystr) throws IOException, ParseException {
