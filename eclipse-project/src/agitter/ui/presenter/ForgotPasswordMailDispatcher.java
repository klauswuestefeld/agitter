...
public class ForgotPasswordMailDispatcher {
	...
	public static void send(User user) throws IOException {
		...
		Destination destination = new Destination(Arrays.asList(user.email()));
		Content subject = new Content(SUBJECT);
		String mailText = BODY.replaceAll("%USERNAME%", user.username()).replaceAll("%PASSWORD%", user.password());
