...
public class ContactChooserHelper {
	...
	private static ProfileListItem createProfile(Group g) {
		return createProfile(g, 0);
	}

	private static ProfileListItem createProfile(Group g, int level) {
		return new ProfileListItem(g.name(), "Grupo " + g.name(), null, level);
	}
	...
	public static ProfileListItem createProfile(User obj, int level) {
		return new ProfileListItem(obj.email().toString(), obj.name(),
				obj.picture(), level);
	}

	public static List<ProfileListItem> contacts(Iterable<Group> groups,
			Iterable<User> contacts) {
		List<ProfileListItem> contactsAndGroups = new ArrayList<ProfileListItem>();

		for (Group g : groups)
			contactsAndGroups.add(createProfile(g));
