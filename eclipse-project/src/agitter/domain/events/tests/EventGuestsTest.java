...
public class EventGuestsTest extends EventsTestBase {
	...
	public void emailInvitation() throws Exception {
		...
		subject.edit(ana, event, "Dinner at Joes", 1000, Collections.EMPTY_LIST, Arrays.asList(user("jose@email.com")));
	...
	public void groupInvitation() throws Exception {
		...
		subject.edit(user, event, "Dinner at Joes", 1000, Arrays.asList(friends), Collections.EMPTY_LIST);
		assertFalse(subject.toHappen(jose).isEmpty());
	}
	...
	public void subgroupInvitation() throws Exception {
		...
		subject.edit(ana, event, "Dinner at Joes", 1000, Arrays.asList(family), Collections.EMPTY_LIST);
		assertTrue(subject.toHappen(jose).isEmpty());

		subject.edit(ana, event, "Dinner at Joes", 1000, Arrays.asList(family, friends), Collections.EMPTY_LIST);
		assertFalse(subject.toHappen(jose).isEmpty());
	}
